...
// 输入数据结构
typedef struct {
    uchar block_header[80];  // 区块头
    uchar target[32];        // 目标难度
    uint nonce_start;        // 起始 nonce
    uint nonce_count;        // nonce 数量
} MiningInput;
...
// SHA256 压缩函数
void sha256_compress(uint* state, const uint* block) {
    uint w[64];
...
// SHA256 哈希计算
void sha256_hash(const uchar* input, uint input_len, uint* output) {
    uint state[8];
    uint block[16];
    ...
    for (uint i = 0; i < remaining; i++) {
        ((uchar*)block)[i] = input[blocks * 64 + i];
    }
    
    // 添加填充
    ((uchar*)block)[remaining] = 0x80;
...
// 检查哈希是否满足目标难度
bool check_target(const uint* hash, const uint* target) {
    for (int i = 7; i >= 0; i--) {
...
__kernel void sha256d_mining(
    ...
    *((uint*)(header + 76)) = nonce;
    ...
    // 第二次 SHA256
    uint final_hash[8];
    sha256_hash((uchar*)first_hash, 32, final_hash);
    
    // 检查是否满足目标难度
    uint target[8];
    for (int i = 0; i < 8; i++) {
        target[i] = *((uint*)(input->target + i * 4));
    }
    
    // 存储结果 (1 表示找到有效解，0 表示无效)
    results[gid] = check_target(final_hash, target) ? 1 : 0;
}
...
__kernel void sha256d_mining_optimized(
    __constant MiningInput* input,
    __global uint* results,
    __local uint* shared_target,
    __local uint* shared_k
) {
    uint gid = get_global_id(0);
    ...
    if (lid == 0) {
        for (int i = 0; i < 8; i++) {
            shared_target[i] = *((uint*)(input->target + i * 4));
        }
        for (int i = 0; i < 64; i++) {
            shared_k[i] = K[i];
        }
    ...
    *((uint*)(header + 76)) = nonce;
    ...
    // 检查目标难度
    results[gid] = check_target(final_hash, shared_target) ? 1 : 0;
}
